...
export const doctorsApi = {
  ...
  deactivateDoctor: async (id: number): Promise<Doctor> => {
    ...
    const response = await fetch(`${API_URL}/doctors/${id}/deactivate`, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export const appointmentsApi = {
  ...
  getAvailableSlots: (date: string, doctorId: number) => 
    api.get<{ time: string; available: boolean }[]>(`/appointments/slots?date=${date}&doctorId=${doctorId}`),
  ...
  updateAppointmentStatus: (id: number, status: 'scheduled' | 'cancelled' | 'completed') =>
